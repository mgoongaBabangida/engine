...
private:
	extremDots dots;
	//bool dynamic = false; ?
	std::vector<dbb::line>	getRays(const ITransform& trans, Side moveDirection, std::vector<float>& lengths)	const;
	void					getForwardRayLengths(const ITransform & trans, Side moveDirection, std::vector<float>& lengths)	const;
};
