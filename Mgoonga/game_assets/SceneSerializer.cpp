...
//-------------------------------------------------------------
void SceneSerializer::Serialize(std::string _filepath)
{
  YAML::Emitter out;
  out << YAML::BeginMap;
...
//-------------------------------------------------------------
std::vector<std::shared_ptr<eObject>> SceneSerializer::Deserialize(std::string _filepath)
{
  ...
  if (serialized_objects)
  {
    ...
    for (auto serialized_object : serialized_objects)
    {
      ...
      std::shared_ptr<eObject> object = factory.CreateObject(m_model_manager.get().Find(model_name), rendering_type, object_name);
